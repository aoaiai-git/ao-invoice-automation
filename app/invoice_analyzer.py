...
添付された請求書PDFを分析し、以下の情報をJSON形式で抽出してください。

必ず以下のJSONキーを含めてください：
- vendor_name: 請求元会社名（文字列）
- invoice_number: 請求書番号（文字列、不明なら空文字）
- invoice_date: 請求日または発行日（YYYY-MM-DD形式、不明なら今日の日付）
- due_date: 支払期限（YYYY-MM-DT形式、不明なら空文字）
- amount_excl_tax: 税抜金額（整数）
- tax_amount: 消費税額（整数）
- amount_incl_tax: 税込金額（整数）
- currency: 通貨コード（JPY, USD等）
- description: 内容・摘要（最大100文字）
- suggested_account: freeeの勘定科目名（下記リストから最適なものを選択）
- suggested_account_id: 上記勘定科目のfreee ID（整数）
...
利用可能な勘定科目：""" + "\n".join([f"- {k}（ID: {v}）" for k, v in ACCOUNT_ITEMS.items()]) + """
...
class InvoiceAnalyzer:
    ...
    async def analyze_invoice(self, pdf_data: bytes, sender: str, subject: str) -> dict:
        ...
        user_message = f"以下の請求書PDFを分析してください。損作列：捖信し{sender}\n件名を: {subject}\nPDFの内容を解析し、指定されたJSON形式で情報を抽出してください"
        try:
            response = self.client.messages.create(model="claude-sonnet-4-6", max_tokens=1024, system=SYSTEM_PROMPT, messages=[{"role": "user", "content": [{"type": "document", "source": {"type": "base64", "media_type": "application/pdf", "data": pdf_b64}}, {"type": "text", "text": user_message}]}])
            ...
            logger.info(f"Invoice analysis: vendor={result.get('vendor_name')}, amount={result.get('amount_incl_tax')}, account={result.get('suggested_account')}")
            return result
